...
FILES = "abcdefgh"
RANKS = "87654321"  # top row is rank 8 after warp
...
def _quad_squareness_score(quad):
    ...
    rightness = 1 - np.mean([abs(cos_angle(q[0],q[1],q[2])),
                              abs(cos_angle(q[1],q[2],q[3])),
                              abs(cos_angle(q[2],q[3],q[0])),
                              abs(cos_angle(q[3],q[0],q[1]))])
...
def detect_and_warp_board(bgr, out_size=800, ensure_a1_dark=True):
    ...
    warped, M = warp_board(bgr, corners, out_size=out_size)
    k = 0
    if ensure_a1_dark:
...
def split_squares(board_img, N=8):
    ...
    for r in range(N):
        for c in range(N):
            y0, y1 = r*s_h, (r+1)*s_h
            x0, x1 = c*s_w, (c+1)*s_w
            squares.append(board_img[y0:y1, x0:x1])
    return squares  # order: a8..h1
...
def squares_with_metadata(board_img, N=8):
    ...
    for r in range(N):
        for c in range(N):
            ...
            x0, y0 = c*s_w, r*s_h
            x1, y1 = (c+1)*s_w, (r+1)*s_h
            meta.append({"name": name, "rc": (r,c), "xyxy": (x0,y0,x1,y1)})
    return meta
...
def draw_grid_overlay(board_img, line_thickness=2):
    ...
    scale = max(0.4, s_h/160.0); thick = max(1, int(s_h/200))
...
def crop_by_name(board_img, name):
    name = name.lower().strip()
    c = FILES.index(name[0]); r = RANKS.index(name[1])
    h, w = board_img.shape[:2]; s_h, s_w = h//8, w//8
...
def main():
    ap = argparse.ArgumentParser(description="Detect chessboard, warp, draw grid, and optionally save squares.")
    ap.add_argument("image", help="Path to input photo/screenshot")
    ap.add_argument("--size", type=int, default=800, help="Output square size in px")
    ...
    warped, corners, M, k = detect_and_warp_board(img, out_size=args.size, ensure_a1_dark=not args.no_a1_fix)
